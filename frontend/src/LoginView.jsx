...
function BrandPanel() {
  return (
    <aside className="relative overflow-hidden bg-primary p-6 text-primary-content sm:p-8 lg:p-10">
      ...
      <div className="relative flex min-h-full flex-col justify-between gap-10">
        <div>
          <div className="max-w-md">
            <div className="grid h-44 w-44 place-items-center rounded-2xl bg-base-100 p-1 shadow-soft ring-1 ring-white/60 sm:h-60 sm:w-60">
...
function LoginView({ onLogin }) {
  ...
  return (
    <div data-theme="usiTheme" className="min-h-screen overflow-hidden bg-base-200 px-4 py-6 text-base-content sm:px-6 lg:py-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col justify-center p-6 sm:p-8 lg:p-10">
            <div>
              <div className="flex items-center gap-3">
                <span className="grid h-14 w-14 place-items-center rounded-xl bg-base-100 p-2 text-secondary shadow-sm ring-1 ring-base-300">
                  <img src={usiLogoIcon} alt="" className="h-full w-full object-contain" aria-hidden="true" />
                </span>
                <div>
                  <h2 className="text-2xl font-black text-primary">Iniciar sesion</h2>
                  <p className="text-sm font-semibold text-base-content/60">Ingresa con la cuenta asignada para tu rol.</p>
                </div>
              </div>
              ...
              <button type="submit" disabled={status === 'loading'} className="btn btn-primary mt-7 w-full normal-case disabled:opacity-50">
                {status === 'loading' ? <Loader2 className="h-4 w-4 animate-spin" /> : <LockKeyhole className="h-4 w-4" />}
                Entrar al sistema
              </button>

              <p className="mt-5 rounded-lg border border-base-300 bg-base-200 px-4 py-3 text-xs font-semibold leading-5 text-base-content/60">
